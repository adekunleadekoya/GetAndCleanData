...
get.tidy.dataset<-function()
		{
			...
			DT<-cbind(subject_DT,DT)  
		      # above line adds a subject column to the left of  data frame DT                
                	colnames(DT)[1]<- 'subject' 
			# above line labels the new  subject column in DT
...
DT=get.tidy.dataset() 
	 ...
	 # the tidy dataset generated by this function is saved inside a file : 'tidyDT.txt'
